#   Distributed Multiagent Q-Learning - 3 blobs - with Obstacle
#   -------------------------------------------
#   Author: Ryan Russell
#   
...
for episode in range(EPISODES):
    ...
    # Get Distance apart from each agent for every agent
        obs_1 = [A[0][2]*(Agent1.x - Agent3.x), A[0][2]*(Agent1.y - Agent3.y), Agent1.x - Obstacle1.x, Agent1.y - Obstacle1.y]
        obs_2 = [A[1][0]*(Agent2.x - Agent1.x), A[1][0]*(Agent2.y - Agent1.y), Agent2.x - Obstacle1.x, Agent2.y - Obstacle1.y]
        obs_3 = [A[2][1]*(Agent3.x - Agent2.x), A[2][1]*(Agent3.y - Agent2.y), Agent3.x - Obstacle1.x, Agent3.y - Obstacle1.y]
    ...
    # Take an action for each agent    
        ...
        # See if Agent hit the obstacle
        if Agent1.x - Obstacle1.x == 0 and Agent1.y - Obstacle1.y == 0:
            obstacle_reward_1 = OBSTACLE_REWARD
        else:
            obstacle_reward_1 = 0
        if Agent2.x - Obstacle1.x == 0 and Agent2.y - Obstacle1.y == 0:
            obstacle_reward_2 = OBSTACLE_REWARD
        else:
            obstacle_reward_2 = 0
        if Agent3.x - Obstacle1.x == 0 and Agent3.y - Obstacle1.y == 0:
            obstacle_reward_3 = OBSTACLE_REWARD
        else:
            obstacle_reward_3 = 0
        
        r_1 = 1 - A[0][1]*np.sqrt(np.power(Agent1.x-Agent2.x,2)+np.power(Agent1.y-Agent2.y,2)) - A[0][2]*np.sqrt(np.power(Agent1.x-Agent3.x,2)+np.power(Agent1.y-Agent3.y,2)) - obstacle_reward_1
        r_2 = 1 - A[1][0]*np.sqrt(np.power(Agent2.x-Agent1.x,2)+np.power(Agent2.y-Agent1.y,2)) - A[1][2]*np.sqrt(np.power(Agent2.x-Agent3.x,2)+np.power(Agent2.y-Agent3.y,2)) - obstacle_reward_2
        r_3 = 1 - A[2][0]*np.sqrt(np.power(Agent3.x-Agent1.x,2)+np.power(Agent3.y-Agent1.y,2)) - A[2][1]*np.sqrt(np.power(Agent3.x-Agent2.x,2)+np.power(Agent3.y-Agent2.y,2)) - obstacle_reward_3
        
        new_obs_1 = [A[0][2]*(Agent1.x - Agent3.x), A[0][2]*(Agent1.y - Agent3.y), Agent1.x - Obstacle1.x, Agent1.y - Obstacle1.y]
        new_obs_2 = [A[1][0]*(Agent2.x - Agent1.x), A[1][0]*(Agent2.y - Agent1.y), Agent2.x - Obstacle1.x, Agent2.y - Obstacle1.y]
        new_obs_3 = [A[2][1]*(Agent3.x - Agent2.x), A[2][1]*(Agent3.y - Agent2.y), Agent3.x - Obstacle1.x, Agent3.y - Obstacle1.y]    
        ...
        if show:
            ...
            env[Obstacle1.y][Obstacle1.x] = d[OBSTACLE_N]
